  ...
  render() {
    ...
    switch (index) {
      ...
      case "memoire":
        content = {
          title: data.TICO,
          subtitle: joinData([data.OBJET, data.EDIF, data.LEG, data.DATOEU, data.DATOEU ? "" : data.SCLE]),
          ref: data.REF,
    ...
    return (
      <Link style={{ textDecoration: "none" }} to={`/notice/${index}/${data.REF}`} className="map-card" key={data.REF}>
