...
export default class Umbrella extends React.Component {
  ...
  onMapChange(boxZoomBounds) {
    ...
    this.updateQuery(boxZoomBounds.north, boxZoomBounds.west, boxZoomBounds.south, boxZoomBounds.east, precision);
    //}
  }
...
class Map extends React.Component {
  ...
  addSourceAndLayers = () => {
    ...
    this.map.addLayer({
      ...
      filter: ["!", ["has", "count"]],
      paint: {
        "circle-color": ["case", ["boolean", ["feature-state", "clicked"], false], "#fff", "#9C27B0"],
        "circle-radius": 9,
        "circle-stroke-width": 2,
        "circle-stroke-color": ["case", ["boolean", ["feature-state", "clicked"], false], "#9C27B0", "#fff"],
      },
    });
  ...
  onPointClicked(e, layerType) {
    ...
    if (features.length === 1) {
      ...
      if (layerType === "clusters") {
        ...
        drawerContent = <SingleNotice className="" key={item.REF} data={item} onClose={this.closeDrawer} />;
      }
    ...
    return (
      <div style={style} className="search-map">
        ...
        <div id="map" ref={this.mapRef} style={style}>
          <div className={`drawer ${this.state.drawerContent ? "open" : ""}`}>{this.state.drawerContent}</div>
